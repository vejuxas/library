...
-- Player Notifications Toggle
Tabs.Settings:AddToggle("PlayerNotifications", {
    Title = "Player Notifications",
    Description = "Show notifications when players join/leave",
    Default = false,
    Callback = function(Value)
        showPlayerNotifications = Value
        print("Player notifications:", Value)
    end
})

-- Keybind variables
local uiKeybind = UserInputService.TouchEnabled and "Space" or "RightShift"
local speedKeybind = "F1"
local flyKeybind = "F2"
local noclipKeybind = "F3"

-- Notification settings
local showPlayerNotifications = false -- Off by default
...
Tabs.Settings:AddSlider("ESPUpdateRate", {
    Title = "ESP Update Rate",
    Description = "How often ESP updates (higher = better performance)",
    Default = 1,
    Min = 1,
    Max = 10,
    Rounding = 1,
    Callback = function(val)
        espUpdateRate = val
        print("ESP update rate set to:", val, "frames")
    end
})
...
Tabs.Settings:AddToggle("PerformanceMode", {
    ...
    Default = false,
    Callback = function(val)
        print("Performance mode:", val)
        Fluent:Notify({
            Title = "Performance Mode",
            Content = val and "Performance mode enabled - reduced effects" or "Performance mode disabled - full effects",
            Duration = 3
        })
    end
})
...
Tabs.Settings:AddKeybind("SpeedToggle", {
    Title = "Speed Toggle",
    Mode = "Toggle",
    ...
    Callback = function(Value)
        ...
        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            if speedEnabled then
                if speedConnection then speedConnection:Disconnect() end
                speedConnection = game:GetService("RunService").Heartbeat:Connect(updateSpeed)
                print("Speed enabled via keybind!")
            else
                if speedConnection then 
                    speedConnection:Disconnect() 
                    speedConnection = nil
                end
                print("Speed disabled via keybind!")
            end
        end
        print("Speed toggled via keybind:", speedEnabled)
    end
})

-- Fly Toggle Keybind
Tabs.Settings:AddKeybind("FlyToggle", {
    Title = "Fly Toggle",
    Mode = "Toggle",
    Default = flyKeybind,
    Callback = function(Value)
        ...
        print("Fly toggled via keybind:", flyEnabled)
    end
})

-- Noclip Toggle Keybind
Tabs.Settings:AddKeybind("NoclipToggle", {
    Title = "Noclip Toggle",
    Mode = "Toggle",
    ...
    Callback = function(Value)
        ...
        print("Noclip toggled via keybind:", noclipEnabled)
    end
})
...
local originalSizes = {}
local hitboxColor = Color3.fromRGB(0, 255, 0) -- Default green color
...
local function updateHitbox()
    ...
    if not hitboxUpdateConnection then
        hitboxUpdateConnection = game:GetService("RunService").Heartbeat:Connect(function()
            -- Update rainbow hue (slower for smoother transitions)
            if rainbowHitbox then
...
-- Rainbow Hitbox Toggle
Tabs.Hitbox:AddToggle("RainbowHitbox", {
    Title = "Rainbow Hitbox",
    Description = "Animated rainbow color effect",
    Default = false,
    Callback = function(val)
        rainbowHitbox = val
        if hitboxEnabled then
        ...
        print("Rainbow Hitbox:", val)
    end
})

-- Team Check Toggle
Tabs.Hitbox:AddToggle("TeamCheck", {
    Title = "Team Check",
    Description = "Don't expand hitboxes for teammates",
    Default = false,
    Callback = function(val)
        teamCheck = val
        if hitboxEnabled then
            -- Restore teammates' hitboxes if enabled
            if val then
                for _, player in pairs(Players:GetPlayers()) do
                    if player.Team == LocalPlayer.Team and player.Team ~= nil and player ~= LocalPlayer then
                        restorePlayerHitbox(player)
                    end
                end
            end
...
-- Auto-update player list function
local function updatePlayerDropdown()
    local currentPlayers = getPlayerNames()
    -- Note: Fluent UI doesn't have direct dropdown update, so we'll use notifications
    print("Player list updated! Current players:", #currentPlayers)
end
...
Players.PlayerAdded:Connect(function(player)
    ...
    if showPlayerNotifications then
        Fluent:Notify({
            Title = "Player Joined",
            Content = player.Name .. " joined the game. Player list updated!",
            Duration = 3
        })
    end
end)
...
Players.PlayerRemoving:Connect(function(player)
    ...
    -- Clear selection if the selected player left
    if selectedPlayer == player then
        selectedPlayer = nil
        cleanupVisuals()
        print("Selected player left, cleared selection")
    end
    ...
    if showPlayerNotifications then
        Fluent:Notify({
            Title = "Player Left",
            Content = player.Name .. " left the game. Player list updated!",
            Duration = 3
        })
    end
end)

-- Add a refresh function that updates the dropdown values
local function refreshPlayerDropdown()
    local currentPlayers = getPlayerNames()
    -- The dropdown will automatically refresh when clicked due to getPlayerNames() being called fresh
    return currentPlayers
end

-- Reset Player List Button
Tabs.Target:AddButton({
    Title = "Reset Player List",
    Description = "Refresh the player list to show current players",
    Callback = function()
        -- Clear current selection if player left
        if selectedPlayer and not Players:FindFirstChild(selectedPlayer.Name) then
            selectedPlayer = nil
            cleanupVisuals()
            print("Selected player left the game, cleared selection")
        end
        
        -- Get fresh player count
        local currentPlayers = getPlayerNames()
        
        print("Player list refreshed! Current players:", #currentPlayers)
        
        -- Show notification with instruction
        Fluent:Notify({
            Title = "Player List",
            Content = "Player list refreshed! Found " .. #currentPlayers .. " players. The dropdown will update when you click on it.",
            Duration = 4
        })
    end
})
...
local espUpdateRate = 1 -- Frames between ESP updates
local espUpdateCounter = 0 -- Current frame counter
...
-- ESP Settings (moved to top to avoid nil reference errors)
    ...
    local c = RunService.RenderStepped:Connect(function()
        if player.Character ~= nil and player.Character:FindFirstChild("Humanoid") ~= nil and player.Character:FindFirstChild("HumanoidRootPart") ~= nil and player.Character.Humanoid.Health > 0 and player.Character:FindFirstChild("Head") ~= nil then
            ...
            if vis then
                ...
                local distance = 0
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    distance = (LocalPlayer.Character.HumanoidRootPart.Position - oripart.Position).magnitude
                end
                local value = distance > 0 and math.clamp(1/distance*100, 1, 4) or 1
...
local function createESP(player)
    ...
    connection = RunService.RenderStepped:Connect(function()
        -- Performance optimization: only update every N frames
        espUpdateCounter = espUpdateCounter + 1
        if espUpdateCounter < espUpdateRate then
            return
        end
        espUpdateCounter = 0
                ...
                if lineESPEnabled and library.line then
                    ...
                    if head then
                        ...
                        if OnScreen then
                            ...
                            local thickness = distance > 0 and math.clamp(1/distance*100, 0.1, 3) or 0.1
