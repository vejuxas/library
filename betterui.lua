...
function Compkiller:_Animation(Self: Instance , Info: TweenInfo , Property :{[K] : V})
	...
	local Tween = TweenService:Create(Self , Info or TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out) , Property);
	...
	if Name == "Toggle" then
		...
		local ToggleElement = function(bool,noChange)
			...
			if bool then
				...
				-- Bouncy animation for satisfying toggle feel
				Compkiller:_Animation(ToggleValue,TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out),{
					Position = UDim2.new(0.75, 0, 0.5, 0)
				})

				Compkiller:_Animation(Toggle,TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{
					BackgroundColor3 = Compkiller.Colors.Toggle
				})
			else
				Toggle:SetAttribute('Enabled',false);

				Compkiller:_Animation(ToggleValue,TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),{
		...
		-- Improved hover animation for better visual feedback
		Compkiller:_Hover(Input , function()
			if not Default then
				Compkiller:_Animation(ToggleValue,TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{
...
function Compkiller:_CreateBlock(Signal)
	...
	BlockText.ZIndex = 10
	BlockText.Font = Enum.Font.GothamSemibold  -- Enhanced font weight for better readability
	BlockText.Text = "Block"
	BlockText.TextColor3 = Compkiller.Colors.SwitchColor
	BlockText.TextSize = 13.000  -- Slightly smaller for cleaner look
	BlockText.TextTransparency = 0.200  -- More opaque for better contrast
	BlockText.TextXAlignment = Enum.TextXAlignment.Left
...
function Compkiller:_AddColorPickerPanel(Button: ImageButton , Callback: (Color: Color3) -> any?)
	...
	-- Enhanced color picker with ultra-smooth animations
	function Args:Update()
		local MainColor = Color3.fromHSV(H , S , 1);
		local RealColor = Color3.fromHSV(H , S , V);

		Compkiller:_Animation(ColorPickBox,TweenInfo.new(0.15, Enum.EasingStyle.Sine, Enum.EasingDirection.Out),{
			BackgroundColor3 = Color3.fromHSV(H , 1 , 1)
		});

		Compkiller:_Animation(ColorOpc,TweenInfo.new(0.15, Enum.EasingStyle.Sine, Enum.EasingDirection.Out),{
			BackgroundColor3 = RealColor
		});

		Compkiller:_Animation(MouseMovement,TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{
			Position = UDim2.fromScale(S , 1 - V)
		});

		Compkiller:_Animation(ColorOptSlide,TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{
			Position = UDim2.new(Transparency ,0 , 0.5 ,0)
		});

		Compkiller:_Animation(ColorRGBSlide,TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{
...
function Compkiller:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal)
	...
	function Args:AddSlider(Config: Slider)
		...
		-- Enhanced slider hover with glow effect
		...
		-- Enhanced slider update with smooth spring animation
		local Update = function(Input)
			...
			-- Smoother slider animation with spring easing
			TweenService:Create(SliderInput , TweenInfo.new(0.25, Enum.EasingStyle.Sine, Enum.EasingDirection.Out),{
	...
	function Args:AddDropdown(Config : Dropdown)
		...
		-- Enhanced dropdown hover with smooth color transition
		Compkiller:_Hover(ValueItems,function()
			Compkiller:_Animation(ValueItems,TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{
...
function Compkiller.new(Config : Window)
	...
	-- Enhanced tab expansion with smoother, more elegant animation
	TabHover:Connect(function(value)
		local Style = TweenInfo.new(0.5, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out);
	...
	function WindowArgs:DrawTab(TabConfig : TabConfig , Internal)
		...
		function TabArgs:DrawSection(config: Section)
			...
			local refresh = function(Upvalue)
				...
				-- Enhanced section expand/collapse with smoother spring animation
				if IsOpen then
					...
					if (Section:GetAttribute('Height') and not Compkiller:_IsMobile() and FullScale <= RefPos) then
						Compkiller:_Animation(Section,TweenInfo.new(0.45,Enum.EasingStyle.Exponential,Enum.EasingDirection.Out),{
					...
					Compkiller:_Animation(Section,TweenInfo.new(0.4,Enum.EasingStyle.Exponential,Enum.EasingDirection.Out),{
			...
			-- Enhanced section header hover for better interactivity
			Header.MouseEnter:Connect(function()
				Compkiller:_Animation(SectionText,TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{
					TextTransparency = 0.1
				})
				Compkiller:_Animation(SectionClose,TweenInfo.new(0.2),{
...
function Compkiller:Loader(IconId,Duration)
	...
	-- Enhanced loader with cinematic fade-in
	Compkiller:_Animation(Loader,TweenInfo.new(0.6,Enum.EasingStyle.Exponential,Enum.EasingDirection.Out),{
	...
	task.delay(0.4,function()
		...
		task.delay(0.3,function()
			...
			-- Smooth zoom-out exit
			Compkiller:_Animation(Vignette,TweenInfo.new(2.5,Enum.EasingStyle.Exponential,Enum.EasingDirection.Out),{
				Size = UDim2.new(2.5, 0, 2.5, 0)
			});

			Compkiller:_Animation(Icon,TweenInfo.new(0.8,Enum.EasingStyle.Exponential,Enum.EasingDirection.In),{
				ImageTransparency = 1,
			});

			Compkiller:_Animation(Loader,TweenInfo.new(1.2,Enum.EasingStyle.Exponential,Enum.EasingDirection.InOut),{
				BackgroundTransparency = 1
			});
...
function Compkiller.newNotify()
	...
	Compkiller.NOTIFY_CACHE = {
		...
		Header.TextXAlignment = Enum.TextXAlignment.Left
			...
			-- Enhanced notification animations with elegant slide effects
			local Close = function()
				Compkiller:_Animation(NotifyFrame,TweenInfo.new(0.5,Enum.EasingStyle.Back,Enum.EasingDirection.In),{
					Position = UDim2.new(1,200,0,0)
				});

				task.wait(0.25);

				Compkiller:_Animation(BlockFrame,TweenInfo.new(0.25,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{
			...
			local Show = function()
				-- Smooth bounce-in effect for notifications
				Compkiller:_Animation(NotifyFrame,TweenInfo.new(0.6,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{
					Position = UDim2.new(0,0,0,0)
				});
			end;
